 ...
 * and handles search highlighting and dropzone visual states based on the global props provided to RichTree.
 */
const TreeItemComponent = <R extends TreeViewBaseItem>(
  { highlightedLabelSegment, isDropzoneActive, itemId, resolvers, slotProps, slots, variant = 'standard', ...restProps }: TreeItemProps<R>,
  ...
  const resolvedProps = resolvers
    ? {
        ...
        LabelTypographyProps: resolvers.getLabelTypographyProps?.(treeItemModel),
  ...
  const labelSlotProps: Partial<TreeItemLabelWithIconProps> | undefined = {
    ...
    // Does not affect label editing, isItemEditable prop on RichTree serves for that
    editable: undefined,
    highlightedLabelSegment,
  };

  const checkboxSlotProps: SlotComponentProps<ElementType<typeof Checkbox>, object, object> | undefined = {
    ...slotProps?.checkbox,
    ...CheckboxProps,
  };
...
export const TreeItem = Forwarded as typeof Forwarded & {
  <R extends TreeViewBaseItem>(props: TreeItemProps<R> & { ref?: Ref<HTMLLIElement> }): ReturnType<typeof TreeItemComponent>;
};
