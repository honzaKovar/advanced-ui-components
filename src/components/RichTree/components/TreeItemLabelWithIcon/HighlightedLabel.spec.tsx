...
const renderComponent = (labelText: string, highlightedSegment: string) =>
  render(
    <ThemeProvider>
      <HighlightedLabel labelText={labelText} normalizedHighlightedSegment={highlightedSegment.toLowerCase()} />
    </ThemeProvider>,
  );
...
describe('HighlightedLabel', () => {
  describe('Rendering Logic', () => {
    ...
    it('should correctly highlight multiple occurrences of the same segment', () => {
      ...
      const highlightedSpans = Array.from(container.querySelectorAll(mockHighlightClass));
    ...
    it('should preserve and highlight trailing whitespace in the segment', () => {
      ...
      const { container } = renderComponent(labelText, highlightedSegmentWithTrailingSpace);
