...
export type TreeViewBaseItem<
  R extends NonNullable<unknown> = TreeViewDefaultItemModelProperties,
> = R & {
...
export type TreeItemPropResolvers<R> = {
  ...
  getTreeItemProps?: (
    item: R,
  ) => { hasDropzone?: boolean; sx?: SxProps<Theme> } | undefined;
  isItemDisabled?: (item: R) => boolean;
};

export type TreeItemProps<R extends TreeViewBaseItem = TreeViewBaseItem> =
  MuiTreeItemProps & {
...
export type RichTreeViewProWrapperProps<
  R extends TreeViewBaseItem,
  Multiple extends boolean | undefined,
> = Omit<RichTreeViewProProps<R, Multiple>, "items"> &
  Pick<
    TreeItemProps,
    "isDropzoneActive" | "highlightedLabelSegment" | "variant"
  > & {
    items: readonly R[];
    resolvers?: TreeItemPropResolvers<R>;
    sx?: SxProps<Theme>;
