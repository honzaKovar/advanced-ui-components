...
const getDropzoneGradient = (palette: Theme["palette"]) => {
  ...
  return angles
    .map(
      (angle) =>
        `repeating-linear-gradient(${angle}deg, ${color} 0 ${dash}px, transparent ${dash}px ${
          dash + gap
        }px)`,
    )
    .join(", ");
...
export const getDefaultTreeItemSx = (
  variant: TreeItemVariant,
  showDropzone?: boolean,
): SxProps<Theme> =>
  mergeSx(
    commonTreeItemSx,
    variant === "outlined" ? outlinedTreeItemSx : {},
    showDropzone ? dropzoneSx : {},
  );
