...
export const TreeItemLabelWithIcon = ({
  ...
  const { sx: labelSx, ...restLabelTypographyProps } =
    LabelTypographyProps ?? {};
  const {
    Icon: StartIcon,
    IconProps: StartSvgIconProps,
    show: showStartIcon,
  ...
  const { fontSize: startIconSize = "xSmall", ...restStartSvgIconProps } =
    StartSvgIconProps ?? {};
  const {
  ...
  const renderContent = () => {
    ...
    const foundMatchToHighlight = children
      .toLowerCase()
      .includes(normalizedHighlightedSegment);
    ...
    return (
      <HighlightedLabel
        labelText={children}
        normalizedHighlightedSegment={normalizedHighlightedSegment}
      />
    );
  };
  ...
  return (
    <Stack sx={wrapperSx} {...restProps}>
      {showStartIcon && StartIcon && (
        <StartIcon
          data-testid={startIconTestId}
          fontSize="small"
          {...restStartSvgIconProps}
        />
      )}
      <Typography
        sx={mergeSx(defaultLabelSx, labelSx)}
